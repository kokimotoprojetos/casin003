...
<head>
    <script>
        (function() {
            // Trava de URL Global para Afiliados
            const currentUrl = new URL(window.location.href);
            const pid = currentUrl.searchParams.get('pid');
            
            // Se estiver em uma subpasta como /launch ou /spread, ou se o PID estiver duplicado
            if (window.location.pathname !== '/' || (window.location.search.match(/pid=/g) || []).length > 1) {
                if (pid) {
                    // Redireciona para a raiz mantendo apenas o primeiro PID limpo
                    const cleanPid = pid.split('?')[0].split('/')[0];
                    window.location.replace(window.location.origin + '/?pid=' + cleanPid);
                }
            }
            
            // Interceptar mudanças de URL via History API (para SPAs)
            const originalPushState = history.pushState;
            const originalReplaceState = history.replaceState;
            
            history.pushState = function() {
                const url = arguments[2];
                if (url && (url.includes('/launch') || url.includes('/spread') || url.includes('/subscribeReward'))) {
                    const newUrl = url.replace(/\/(launch|spread|subscribeReward)/g, '');
                    arguments[2] = newUrl;
                }
