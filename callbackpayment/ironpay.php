...
/**
 * CALLBACK IRONPAY
 * Webhook para processar notificações de depósitos e saques da IronPay
 */
...
function log_ironpay($msg, $context = []) {
    $logFile = dirname(__DIR__) . '/errorlog.log';
    $line = '[IRONPAY ' . date('Y-m-d H:i:s') . '] ' . $msg;
    if (!empty($context)) {
...
log_ironpay("Webhook recebido", $data);

function verificarBonusIron($userId, $valorPago, $transacao_id = null) {
    global $mysqli;
...
function registrarBonusUsadoIron($userId, $valorPago, $bonusRecebido) {
    global $mysqli;
...
function buscarValorIpnCashinIron($transacao_id) {
    global $mysqli;
    ...
    if ($res->num_rows > 0) {
        ...
        $bonusRecebido = verificarBonusIron($usuario_id, $valorPago, $transacao_id);
        ...
        if ($retorna_insert_saldo) {
            ...
            if ($bonusRecebido > 0) {
                registrarBonusUsadoIron($usuario_id, $valorPago, $bonusRecebido);
            }
...
function attPaymentPixIron($transacao_id) {
    ...
    if ($sql->execute()) {
        ...
        if ($linhas_afetadas > 0) {
            if (buscarValorIpnCashinIron($transacao_id)) return 1;
            else return 0;
        }
    } else {
        $sql->close();
...
function attSaqueStatusIron($transaction_id, $status) {
    global $mysqli;
...
if (!empty($transaction_id)) {
    if (in_array($status, $valid_success)) {
        // Tentar deposito primeiro
        $res = attPaymentPixIron($transaction_id);
        if ($res == 1 || $res == 2) {
            log_ironpay("Sucesso processamento depósito: $transaction_id ($res)");
            http_response_code(200);
            echo json_encode(['success' => true, 'message' => 'Processed deposit']);
            exit;
        } else {
            // Tentar saque
            $resSaque = attSaqueStatusIron($transaction_id, $status);
            if ($resSaque == 1 || $resSaque == 2) {
                log_ironpay("Sucesso processamento saque: $transaction_id ($resSaque)");
                http_response_code(200);
        ...
        attSaqueStatusIron($transaction_id, $status);
        http_response_code(200);
