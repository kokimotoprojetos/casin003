...
/**
 * CALLBACK GREEPAY
 * Webhook para processar notificações de Cash-In (depósitos) e Cash-Out (saques) da GreePay
 * 
...
log_greepay("Webhook recebido", $data);
...
function log_greepay($msg, $context = []) {
    $logFile = dirname(__DIR__) . '/errorlog.log';
    $line = '[GREEPAY ' . date('Y-m-d H:i:s') . '] ' . $msg;
    if (!empty($context)) {
...
function buscarValorIpnCashin($transacao_id)
{
    ...
    if ($res->num_rows > 0) {
        ...
        if ($retorna_insert_saldo) {
            ...
            log_greepay("Chamando processarTodasComissoes para User $usuario_id, Valor $valorPago");
...
function processarWebhookCashin($webhook_data)
{
    ...
    $transaction_id = PHP_SEGURO($webhook_data['transactionId'] ?? ($webhook_data['transaction_id'] ?? ''));
    $external_id = PHP_SEGURO($webhook_data['externalId'] ?? ($webhook_data['external_id'] ?? ''));
    ...
    // Aceitar múltiplos status de confirmação (incluindo COMPLETE da GGPIX)
    $valid_statuses = ['confirmed', 'paid', 'success', 'completed', 'approved', 'complete'];
    // Normalizar tipo (aceitar variações comuns, incluindo PIX_IN da GGPIX)
    $type_norm = str_replace(['_', '-'], '', $type);
    $is_cashin = in_array($type_norm, ['cashin', 'deposit', 'pix', 'payment', 'pixin']);

    $id_busca = '';
    foreach ([$transaction_id, $request_number, $out_trade_no, $order_no, $external_id] as $cand) {
        if (!empty($cand)) { $id_busca = $cand; break; }
    }

    log_greepay('[GREEPAY CASHIN] Payload recebido', [
        'type' => $type,
        'status' => $status,
        'id_candidates' => [
    ...
    if ($is_cashin && !empty($id_busca) && in_array($status, $valid_statuses)) {
        ...
        if ($att_transacao == 1) {
            log_greepay('[GREEPAY CASHIN] Pagamento processado', ['id' => $id_busca]);
            ...
            if ($stmt) {
                ...
                if ($res && $row = $res->fetch_assoc()) {
                    ...
                    log_greepay('[GREEPAY CASHIN] Reentrega: webhook Pix pago reenviado', ['id' => $id_busca]);
                }
                $stmt->close();
            }
            return ['success' => true, 'message' => 'Pagamento já processado'];
        } else {
            log_greepay('[GREEPAY CASHIN] Erro ao processar pagamento', ['id' => $id_busca]);
            return ['success' => false, 'message' => 'Erro ao processar'];
        }
    } else {
        log_greepay('[GREEPAY CASHIN] Ignorado', [
            'is_cashin' => $is_cashin,
...
function attSaqueStatus($transaction_id, $status, $e2e = '')
{
    ...
    if (!$stmt_find) {
        error_log("[GREEPAY CASHOUT] Erro ao preparar query de busca");
        return 0;
    }
    ...
    if ($res_find->num_rows === 0) {
        $stmt_find->close();
        error_log("[GREEPAY CASHOUT] Saque não encontrado: $transaction_id / $e2e");
    ...
    if ($status === 'completed' && $status_atual == 1) {
        error_log("[GREEPAY CASHOUT] Saque já processado: $transacao_id_real");
        return 2; // Já processado
    }
    
    // Mapear status da GreePay/GGPIX para status no banco
    ...
    if (!$stmt_update) {
        error_log("[GREEPAY CASHOUT] Erro ao preparar query de atualização");
        return 0;
    }
    ...
    if ($stmt_update->execute()) {
        ...
        if ($linhas_afetadas > 0) {
            error_log("[GREEPAY CASHOUT] Saque atualizado com sucesso: $transacao_id_real - Status: $status_db");
            ...
            error_log("[GREEPAY CASHOUT] Nenhuma linha afetada: $transacao_id_real");
            return 0;
        }
    } else {
        error_log("[GREEPAY CASHOUT] Erro ao executar update");
        $stmt_update->close();
        return 0;
    }
}
...
function processarWebhookCashout($webhook_data)
{
    ...
    error_log("[GREEPAY CASHOUT] Webhook recebido - Type: $type, Status: $status, TxID: $transaction_id, ExternalId: $external_id, E2E: $e2e");

    // Validar que é um cashout (aceitar cashout, withdraw, PIX_OUT)
    $type_norm = strtolower(str_replace(['_', '-'], '', $type));
    if (!in_array($type_norm, ['cashout', 'withdraw', 'pixout'])) {
        error_log("[GREEPAY CASHOUT] Tipo inválido: $type");
        return ['success' => false, 'message' => 'Tipo de webhook inválido'];
    }

    // Validar que há um identificador
    if (empty($transaction_id) && empty($e2e) && empty($external_id)) {
        error_log("[GREEPAY CASHOUT] Sem identificador válido");
        return ['success' => false, 'message' => 'Transaction ID, ExternalId ou E2E não informado'];
    }

    // Preferir externalId (que é o nosso transacao_id) na busca; depois transactionId; depois e2e
    $id_busca = !empty($external_id) ? $external_id : (!empty($transaction_id) ? $transaction_id : $e2e);
...
$typeNorm = strtolower(str_replace(['_', '-'], '', $type)); // PIX_IN -> pixin, PIX_OUT -> pixout
if (in_array($typeNorm, ['cashin', 'deposit', 'pix', 'payment', 'pixin'])) {
    $resultado = processarWebhookCashin($data);
} elseif (in_array($typeNorm, ['cashout', 'withdraw', 'pixout'])) {
    $resultado = processarWebhookCashout($data);
} else {
    error_log("[GREEPAY] Tipo de webhook desconhecido: " . json_encode($data));
