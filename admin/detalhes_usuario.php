...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_rollover'])) {
        $action = $_POST['action_rollover'];
        $id_user_int = intval($id_user);
        
        if ($action === 'remover') {
            $mysqli->query("UPDATE audit_flows SET status = 'completed', current_flow = need_flow WHERE user_id = $id_user_int AND status != 'completed'");
            $mysqli->query("UPDATE usuarios SET rev = 0, total_rev = 0 WHERE id = $id_user_int");
            $toastType = 'success';
            $toastMessage = 'Rollover zerado com sucesso!';
        } elseif ($action === 'adicionar') {
            $valor = floatval($_POST['valor_rollover']);
            $res_check = $mysqli->query("SELECT id FROM audit_flows WHERE user_id = $id_user_int AND status != 'completed' LIMIT 1");
            if ($res_check->num_rows > 0) {
                $row = $res_check->fetch_assoc();
                $mysqli->query("UPDATE audit_flows SET need_flow = need_flow + $valor WHERE id = " . $row['id']);
            } else {
                $mysqli->query("INSERT INTO audit_flows (user_id, amount, flow_multiple, need_flow, current_flow, status, flow_type, created_at) VALUES ($id_user_int, $valor, 1, $valor, 0, 'notStarted', 'RECHARGE', NOW())");
            }
            $toastType = 'success';
            $toastMessage = 'Rollover adicionado com sucesso!';
        }
        // Recarregar dados do usuário
        $res = $mysqli->query("SELECT * FROM usuarios WHERE id = $id_user_int");
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-5 ms-auto align-self-center">
                                        <div class="d-flex dash justify-content-center">
                                            ...
                                            <?php
                                            $qry_roll = "SELECT SUM(need_flow - current_flow) as falta FROM audit_flows WHERE user_id = '{$data['id']}' AND status != 'completed'";
                                            $res_roll = mysqli_query($mysqli, $qry_roll);
                                            $roll_data = mysqli_fetch_assoc($res_roll);
                                            $falta_rollover = $roll_data['falta'] > 0 ? $roll_data['falta'] : 0;
                                            ?>
                                            <div class="border-dashed rounded border-theme-color p-2 me-2 flex-grow-1 flex-basis-0">
                                                <h5 class="fw-semibold fs-22 mb-1 text-warning">
                                                    R$<?= number_format($falta_rollover, 2, ',', '.'); ?>
                                                </h5>
                                                ...
                                                <script>
                                                function promptRollover(form) {
                                                    let v = prompt("Valor do rollover:");
                                                    if(!v || isNaN(v.replace(',','.'))) return false;
                                                    form.querySelector('#input_valor_roll').value = v.replace(',','.');
                                                    return true;
                                                }
                                                </script>
                                            </div>
    ...
    <div id="toastPlacement" class="toast-container position-fixed bottom-0 end-0 p-3"></div>
        ...
        <script>
            function acaoRollover(action) {
                let valor = 0;
                if (action === 'adicionar') {
                    let input = prompt("Digite o valor do rollover a adicionar (ex: 100.00):");
                    if (input === null || input === "") return;
                    valor = parseFloat(input.replace(',', '.'));
                    if (isNaN(valor)) {
                        alert("Por favor, digite um valor numérico válido.");
                        return;
                    }
                } else {
                    if (!confirm("Tem certeza que deseja ZERAR o rollover deste usuário?")) return;
                }

                fetch('fetch/editar_rollover.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ user_id: <?= intval($id_user); ?>, action: action, valor: valor })
                })
                .then(response => {
