...
if (!function_exists('daanrox_log')) {
    function daanrox_log($mensagem, $tipo = 'INFO') {
        $logFile = dirname(__DIR__, 2) . '/daanrox.txt';
        $date = date('Y-m-d H:i:s');
        $ip = $_SERVER['REMOTE_ADDR'] ?? 'CLI';
        @file_put_contents($logFile, "[$date] [$ip] [$tipo] $mensagem" . PHP_EOL, FILE_APPEND);
    }
}
#=====================================================#
# DATA CONFIG
...
function data_greepay()
{
	global $mysqli;
	$qry = "SELECT * FROM greepay WHERE id=1";
	$res = mysqli_query($mysqli, $qry);
	$data = mysqli_fetch_assoc($res);
	return $data;
}
...
function enviarSaldo($email, $saldo)
{
    ...
    $stmt = $mysqli->prepare("UPDATE usuarios SET saldo = saldo + ? WHERE mobile = ?");
...
function withdrawSaldo($email, $saldo)
{
    ...
    $stmt = $mysqli->prepare("SELECT saldo FROM usuarios WHERE mobile = ?");
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if ($saldoAtual >= $saldo) {
            $stmt2 = $mysqli->prepare("UPDATE usuarios SET saldo = saldo - ? WHERE mobile = ?");
            $stmt2->bind_param("ds", $saldo, $email);
            return $stmt2->execute() ? 1 : 0;
        }
    }
...
function adicionarSaldoUsuario($id_user, $valor) {
    ...
    // Log de entrada
    daanrox_log("adicionarSaldoUsuario() chamada: User=$id_user, Valor=$valor", 'CRUD');
    
    // Primeiro, obter o saldo atual do usuário
    $stmt = $mysqli->prepare("SELECT saldo FROM usuarios WHERE id = ?");
    if (!$stmt) {
        daanrox_log("Erro ao preparar query para obter saldo: " . $mysqli->error, 'ERRO');
        return false;
    }
    ...
    if ($result->num_rows === 0) {
        daanrox_log("Usuário não encontrado: $id_user", 'ERRO');
        return false;
    }
    ...
    daanrox_log("Saldo atual: $saldo_atual, Novo saldo: $novo_saldo", 'CRUD');
    
    // Atualizar o saldo do usuário
    $stmt = $mysqli->prepare("UPDATE usuarios SET saldo = ? WHERE id = ?");
    if (!$stmt) {
        daanrox_log("Erro ao preparar query para atualizar saldo: " . $mysqli->error, 'ERRO');
        return false;
    }
    ...
    if ($stmt->execute()) {
        daanrox_log("Saldo adicionado com sucesso para usuário $id_user: +$valor", 'CRUD');
        
        // ⭐ CORREÇÃO 1: Converter valor para centavos (INTEIRO) ⭐
        $valor_centavos = intval($valor * 100); // Ex: 30.00 → 3000
        
        // ⭐ CORREÇÃO 2: Remover observacao da query (não existe na tabela) ⭐
        $data_registro = date('Y-m-d H:i:s');
        $stmt_log = $mysqli->prepare("INSERT INTO adicao_saldo (id_user, valor, tipo, data_registro) VALUES (?, ?, ?, ?)");
        
        if ($stmt_log) {
            $tipo = "deposito_pix";
            $stmt_log->bind_param("iiss", $id_user, $valor_centavos, $tipo, $data_registro);
            
            if ($stmt_log->execute()) {
                daanrox_log("Log de adição de saldo registrado para usuário $id_user (centavos: $valor_centavos)", 'CRUD');
            } else {
                daanrox_log("Erro ao registrar log de adição: " . $stmt_log->error, 'ERRO');
            }
            $stmt_log->close();
        } else {
            daanrox_log("Erro ao preparar log de adição: " . $mysqli->error, 'ERRO');
        }
        
        return true;
    } else {
        daanrox_log("Erro ao atualizar saldo do usuário $id_user: " . $mysqli->error, 'ERRO');
        return false;
    }
}
