...
class MultiGatewayProcessor {
    ...
    public function processGreePay($config, $transacao_id, $valor, $chavepix, $cpf, $nome_real) {
        if (empty($config['client_id'])) {
            return ['success' => false, 'message' => 'GGPIX: API Key não configurada'];
        }
        ...
        $this->debug[] = "GGPIX: Tipo: $pix_type, Chave: $pix_key_formatada";
        ...
        $url = $api_url . '/pix/out';
        logAction('GGPIX: Enviando requisição', [
            'url' => $url,
            'payload' => $payload
        ]);
        ...
        if (!$response['success']) {
            logAction('GGPIX: Erro na comunicação', $response);
            return [
                'success' => false,
                'message' => 'GGPIX: Falha de comunicação - ' . ($response['message'] ?? 'sem mensagem'),
                'response' => $response
            ];
        }
        ...
        if ($success_http && !empty($data['id'])) {
            ...
            $this->debug[] = "GGPIX: Saque criado - ID: $e2e";
        } elseif ($http_code === 409) {
            $eh_sucesso = true;
            $e2e = $data['id'] ?? $transacao_id;
            $transaction_id = $data['id'] ?? $transacao_id;
            $this->debug[] = "GGPIX: Saque já existia (409) - tratando como sucesso";
        }

        logAction('GGPIX: Resposta completa', [
            'http_code' => $http_code,
            'success' => $eh_sucesso,
            'e2e' => $e2e,
            'response_data' => $data
        ...
        if ($eh_sucesso) {
            return [
                ...
                'e2e' => $e2e,
                'message' => 'Saque processado via GGPIX',
                'response_data' => $data
            ];
        }
    ...
    public function processPoseidonPay($config, $transacao_id, $valor, $chavepix, $cpf, $nome_real) {
        if (empty($config['client_id']) || empty($config['client_secret'])) {
            return ['success' => false, 'message' => 'PoseidonPay: Credenciais não configuradas'];
        }
        ...
        logAction('PoseidonPay: Enviando requisição de saque', [
            'external_id' => $external_id,
            'url' => $url,
            'payload' => $payload
        ]);
        ...
        if (!$response['success']) {
            logAction('PoseidonPay: Erro na comunicação', $response);
            return [
                'success' => false,
                'message' => 'PoseidonPay: Falha de comunicação - ' . ($response['message'] ?? 'sem mensagem'),
                'response' => $response
            ];
        }
        ...
        if ($http_code >= 200 && $http_code < 300) {
            ...
            logAction('PoseidonPay: Saque aprovado', ['withdrawal_id' => $withdrawal_id]);
            return [
                'success' => true,
                'gateway' => 'poseidonpay',
                'gateway_transaction_id' => $withdrawal_id,
        ...
        $error_msg = $data['message'] ?? $data['error'] ?? 'Erro desconhecido';
        logAction('PoseidonPay: Erro na resposta', ['http_code' => $http_code, 'data' => $data]);
        return [
            'success' => false,
...
// ✅ ORDEM DE PRIORIDADE: GreePay > AurenPay > BSPay > ExpfyPay > PoseidonPay
...
// 1️⃣ Tentar GreePay primeiro (se disponível e preferido ou sem preferência)
if ($greepay_cred && (!$gateway_preferido || $gateway_preferido === 'greepay')) {
    $result = $processor->processGreePay($greepay_cred, $id, $valor, $chavepix, $cpf, $nome_real);
    if (!$result['success']) {
        $errors['greepay'] = $result['message'] ?? 'Falha desconhecida';
        logAction('GreePay falhou, tentando próximo gateway', ['error' => $errors['greepay']]);
    } else {
        logAction('GreePay processou com sucesso', ['gateway_transaction_id' => $result['gateway_transaction_id']]);
    }
}

// 2️⃣ Se GreePay falhou, tentar AurenPay
...
// 5️⃣ Se ExpfyPay falhou, tentar PoseidonPay
if ((!$result || !$result['success']) && $poseidon_cred) {
    $result = $processor->processPoseidonPay($poseidon_cred, $id, $valor, $chavepix, $cpf, $nome_real);
    if (!$result['success']) {
        $errors['poseidonpay'] = $result['message'] ?? 'Falha desconhecida';
        logAction('PoseidonPay falhou', ['error' => $errors['poseidonpay']]);
    } else {
        logAction('PoseidonPay processou com sucesso', ['gateway_transaction_id' => $result['gateway_transaction_id']]);
    }
}
